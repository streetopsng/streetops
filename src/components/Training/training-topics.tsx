...
import { Clock, Target, Lightbulb, Puzzle, User, Scale, ArrowRight } from 'lucide-react';

const topics = [
  {
    title: "Time Management",
    description: "Learn to prioritize tasks and eliminate time-wasters effectively.",
    icon: <Clock className="w-5 h-5 text-red-400" />,
  },
  {
    title: "Goal Setting",
    description: "Master the art of setting and achieving meaningful objectives.",
  ...
  },
  {
    ...
    description: "Create sustainable habits for personal and professional growth.",
  ...
  return (
    <section className="py-8 px-4 bg-white font-sans">
      ...
      <div className="max-w-5xl mx-auto text-center mb-8">
        ...
        <h2 className="text-3xl md:text-4xl font-bold text-red-950 mt-3 mb-2">
          Our Core Training Topics
        </h2>
        <p className="text-gray-600 text-sm max-w-xl mx-auto leading-snug">
          Expert-led modules designed to boost your professional skills and deliver results.
        </p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-5xl mx-auto">
        {topics.map((topic, index) => (
          <div key={index} className="bg-[#FFF9F9] p-5 rounded-2xl flex flex-col items-start border border-red-50/50 hover:shadow-sm transition-shadow">
            <div className="bg-red-100 p-2.5 rounded-lg mb-3">
              {topic.icon}
            </div>
            <h3 className="text-lg font-bold text-gray-800 mb-1.5 leading-tight">
              {topic.title}
            </h3>
            <p className="text-gray-500 text-xs mb-4 flex-grow leading-normal">
              {topic.description}
            </p>
